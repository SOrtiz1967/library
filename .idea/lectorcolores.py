...
#usar pyserial para comunicarse con el arduino 
#agregar una funcion que donde tockjlick me diga que cordenadas son
import numpy as np 
import cv2
import serial, time
...
while(1): 
    ...
    for pic, contour in enumerate(contours): 
        ...
        if area > 300: 
            ...
            # Comprobar si el contorno está en el centro de la cámara
            center_x = x + w // 2
            center_y = y + h // 2
            height, width, _ = imageFrame.shape
            
            # Definir un margen para el área central
            if (width // 3 < center_x < 2 * width // 3) and (height // 3 < center_y < 2 * height // 3):
                print("red")
                
                # Imprimir en la terminal si hay rojo en el centro
